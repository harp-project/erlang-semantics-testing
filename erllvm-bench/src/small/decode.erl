%%----------------------------------------------------------------------
%% decode.erl (new version)
%%----------------------------------------------------------------------
%%    -*- Erlang -*- 
%%    File:	decode.erl  (~jb/decode/decode1.erl)
...
loop(0,R,_) -> R;
loop(N,_,Frame) -> loop(N-1,?MODULE:decode_ie_heads_setup(Frame),Frame).
...
run_orig(N,Frame) ->
   ?MODULE:decode_ie_heads_setup(Frame),
   run_orig(N-1,Frame).
